...
from occrplib.document_classification.document_classifier import \
    DocumentClassifier
from occrplib.document_classification.primarypage_classifier import \
    PrimaryPageClassifier
from occrplib.preprocessing.convert_to_img import (convert_pdf_to_jpg,
                                                   convert_tif_to_jpg)
...
def predict_images_probs(images_path: List[str], binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[dict]:
    ...
    if len(firstpage_prediction_df) > 0:
        ...
        firstpage_prediction_df = pd.concat(
            [firstpage_prediction_df[["file_path"]], document_type_prediction_df], axis=1
        )
    ...
    return df[settings.LABELS_FILTER + ["predicted_label"]].rename(
        columns={"predicted_label": "__predicted"}).to_dict("records")


def predict_image_probs(image_path: str, binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> dict:
...
def predict_documents_probs_with_conversion(
    file_paths: List[str], binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier, convert_to_jpg: Callable[[str, str, bool], None]
) -> List[List[dict]]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir_path:
        ...
        # predict images
        pages_df["predicted_label"] = predict_images_probs(pages_df["path"].to_list(), binary_classifier, document_classifier)
...
def predict_pdfs_probs(pdf_paths: List[str], binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[List[dict]]:
...
def predict_pdf_probs(pdf_path: str, binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[dict]:
...
def predict_tiffs_probs(tiff_paths: List[str], binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[List[dict]]:
...
def predict_tiff_probs(tiff_path: str, binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[dict]:
...
def predict_documents_probs(file_paths: List[str], binary_classifier: PrimaryPageClassifier,
    document_classifier: DocumentClassifier) -> List[List[dict]]:
    ...
    if images:
        images_prediction_df = pd.DataFrame(
            {"file_path": images, "prediction": list(map(lambda p: [p], predict_images_probs(images, binary_classifier, document_classifier)))}
        )
    ...
    if tiffs:
        tiffs_prediction_df = pd.DataFrame({"file_path": tiffs, "prediction": predict_tiffs_probs(tiffs, binary_classifier, document_classifier)})
        predictions.append(tiffs_prediction_df)

    # predict pdf
    pdfs = df[df["extension"] == ".pdf"]["file_path"].to_list()
    if pdfs:
        pdfs_prediction_df = pd.DataFrame({"file_path": pdfs, "prediction": predict_pdfs_probs(pdfs, binary_classifier, document_classifier)})
